        ...
        for in_feature in self.in_features:
            ...
            for k in range(head_length):
                conv = Conv2d(
                    ...
                    stride=1,
                    padding=1,
                    bias=True, # since the given norm is "" in the config, I deleted it -yy
                    norm=get_norm("", conv_dims),
                    activation=F.relu,
                )
...
class DensePoseROIHeads(StandardROIHeads):
    ...
    def forward_with_given_boxes(
        ...
        #print(f'before densepose: {instances}')
        instances = self._forward_densepose(features, instances)
        #print(f'after densepose: {instances}')
        return instances
